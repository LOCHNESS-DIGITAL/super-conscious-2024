...
### Add/remove/modify anything you need in here for your own theme (never modify anything in `rawlins-base`, it will cause git conflicts).
...
function rawlins_unregister_default_widgets(){
...
add_action('widgets_init', 'rawlins_unregister_default_widgets', 11);





function rawlins_theme_setup(){
...
add_action( 'after_setup_theme', 'rawlins_theme_setup' );
...
function rawlins_unregister_tags(){
    unregister_taxonomy_for_object_type('post_tag', 'post');
}
//add_action('init', 'rawlins_unregister_tags');
...
add_action( 'admin_menu', 'rawlins_remove_menus' );
...
function rawlins_taxonomy_checklist_checked_ontop_filter ( $args ){
    $args['checked_ontop'] = false;
    return $args;

}
add_filter('wp_terms_checklist_args','rawlins_taxonomy_checklist_checked_ontop_filter');
...
function rawlins_add_excerpts_to_pages(){
     add_post_type_support( 'page', 'excerpt' );
}
add_action( 'init', 'rawlins_add_excerpts_to_pages' );
...
function rawlins_new_excerpt_more($more) {
    return '&hellip;';
}
add_filter('excerpt_more', 'rawlins_new_excerpt_more');



### Set a custom word length for excerpts.
function rawlins_custom_excerpt_length() {
	return 20;
}
add_filter( 'excerpt_length', 'rawlins_custom_excerpt_length', 999 );
...
function rawlins_search_by_title_only( $search, $wp_query ){
...
if( is_admin() && !wp_doing_ajax() ){
	add_filter('posts_search', __NAMESPACE__ . '\\rawlins_search_by_title_only', 500, 2);
}
...
function rawlins_add_taxonomies_to_pages() {
	register_taxonomy_for_object_type( 'post_tag', 'page' );
	register_taxonomy_for_object_type( 'category', 'page' );
}
// add_action( 'init', 'rawlins_add_taxonomies_to_pages' );

function rawlins_category_and_tag_archives( $wp_query ) {
...
if( !is_admin() ){
	// add_action( 'pre_get_posts', 'rawlins_category_and_tag_archives' );
}
...
function rawlins_remove_type_attr($tag, $handle) {
    return preg_replace( "/type=['\"]text\/(javascript|css)['\"]/", '', $tag );
}
add_filter('style_loader_tag', 'rawlins_remove_type_attr', 10, 2);
add_filter('script_loader_tag', 'rawlins_remove_type_attr', 10, 2);





function rawlins_deregister_scripts(){
  wp_deregister_script( 'wp-embed' );
}
add_action( 'wp_enqueue_scripts', 'rawlins_deregister_scripts' );
 ...
 */
function rawlins_tweak_image_sizes($sizes, $size, $image_src, $image_meta, $attachment_id){
...
add_filter( 'wp_calculate_image_sizes', 'rawlins_tweak_image_sizes', 10, 5);
...
(
    ...
    [image_src] => http://localhost:8888/rawlins-19/web/wp-content/uploads/2018/07/cyclists-hero.png
    [image_meta] => Array
        (
            [width] => 827
            [height] => 1121
...
### Login page
function rawlins_login_logo_url() {
	return home_url();
}
add_filter( 'login_headerurl', 'rawlins_login_logo_url' );

function rawlins_login_logo_url_title() {
	return get_bloginfo( 'name' );
}
add_filter( 'login_headertext', 'rawlins_login_logo_url_title' );
