...
function rawlins_registers_taxonomies(){
    ...
    $rawlins_magic_taxonomy_maker_array = [

        /*
		HOW TO USE

		Copy the array below for 'product-type' and edit as needed. $rawlins_magic_taxonomy_maker_array should be an array of arrays, and those arrays make it easier to create custom taxonomies.
		...
		*/
        ...
        [
			'slug' => 'work-type', // Lowercase letters, dashes only
			'singular' => 'Work Type', // Capitalized, something like 'Product Type' or 'Topic'
			'plural' => 'Work Types', // Capitalized, something like 'Product Types' or 'Topics'
    ...
    foreach( $rawlins_magic_taxonomy_maker_array as $tax_args ){
        ...
        $final_args = rawlins_generate_tax_args_array( $slug, $register_args );

        // Generate the labels and add them to $final_args.
        $labels = rawlins_generate_tax_labels_array([
			'singular' => $singular,
			'plural' => $plural,
		]);
...
add_action( 'init', 'rawlins_registers_taxonomies', 0 );



function rawlins_generate_tax_labels_array( $args = [] ){
...
function rawlins_generate_tax_args_array( $slug, $args = [] ){
